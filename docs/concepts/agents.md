...
| **statusline-setup** | Sonnet | Read, Edit | Configuration de la status line (`/statusline`) |
...
| [Plugin](/concepts/plugins) `agents/` | Projets où le plugin est actif | 4 (plus basse) |
...
├── backend-tasks-executor       # Implementation TDD (acceptEdits)
├── frontend-tasks-planner       # Planification frontend
├── frontend-tasks-executor      # Implementation frontend + design (acceptEdits)
...
Les subagents supportent l'auto-compaction : quand le contexte atteint 95% de capacité, le transcript est automatiquement compressé pour permettre de continuer le travail.

| Aspect | Détail |
|--------|--------|
| Seuil par défaut | 95% de la fenêtre de contexte |
| Override | Variable d'environnement `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` |
| Transcripts | Stockés dans `~/.claude/projects/{project}/{sessionId}/subagents/agent-{agentId}.jsonl` |

### Restreindre les subagents spawnables

Le champ `tools` supporte la syntaxe `Agent(type)` pour limiter quels subagents un agent peut spawner :
...
Dans cet exemple, l'orchestrateur ne peut spawner que les agents `worker` et `researcher`, pas d'autres.

### Désactiver un agent

On peut empêcher Claude d'utiliser un agent spécifique via `permissions.deny` dans les [settings](/concepts/settings) :
